...
Щоденно тягне з Garmin Connect: сон (зокрема фази), HRV, Body Battery, пульс спокою, кроки, стрес, SpO2, тренування за сьогодні (Garmin записує нічний сон поточною датою, тому "сьогодні" — це і є остання завершена ніч) — і пише рядок у Notion-таблицю. Звідти дані читаються через Netlify-проксі в персональному health-брифінгу (`index.html`), а також я (Claude) можу підключитись і робити аналітику/тренди в будь-який момент.
...
Далі воркфлоу ганяється сам щодня о ~8:00 за Києвом (cron `0 6 * * *` / `30 6 * * *` UTC, два запуски — GitHub Actions регулярно затримує перший scheduled-запуск на рідко-активних репо, тому другий про всяк випадок). `morning.py` запускається слідом (о :15 / :45) з невеликим запасом, щоб дані Garmin вже встигли синхронізуватись.
